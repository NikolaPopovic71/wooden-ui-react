...
export function WoodenNumberInput({
  ...
  const propsRef = useRef({ min, max, step, precision, isControlled, onChange })
  ...
  const commit = useCallback((raw) => {
    const { min: mn, max: mx, precision: pr, isControlled: ic, onChange: oc } = propsRef.current
  ...
  return (
    <div className={`wooden-number-wrap ${disabled ? 'is-disabled' : ''}`}>
      ...
      <div className={`wooden-number__field wooden-number__field--${variant}`}>
        ...
        >
          <svg viewBox="0 0 10 2" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <line x1="1" y1="1" x2="9" y2="1" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
          </svg>
        </button>
        ...
        <div className="wooden-number__input-wrap">
          <input
            ...
            type="text"
            inputMode="numeric"
            value={focused ? inputStr : (unit ? `${value}${unit}` : String(value))}
        ...
        >
          <svg viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <line x1="5" y1="1" x2="5" y2="9" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
            <line x1="1" y1="5" x2="9" y2="5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
          </svg>
        </button>
      </div>
      ...
      {(error || helper) && (
        <div className="wooden-number__meta">
          {error ? (
            <span className="wooden-number__error" role="alert">{error}</span>
          ) : (
